...
# input files
# wannier90_hr.dat files for each configuration generated by running wannier90

# output
# configuration_average_hr.dat: hopping parameters averaged over all configurations
...
average_lines.append("Caesar configuration average for hopping elements over {} configurations".format(final_point-first_point+1))
...
for i in range(first_point, final_point+1):
    ...
    if i == first_point:
        ...
        for j in range(read_past):
            average_lines.append(hr_in.readline().rstrip("\n"))
    else:
    ...
    for line in hr_in:
        ...
        if hopping_vector not in configuration_average_dict:
            configuration_average_dict[hopping_vector] = np.zeros((num_wann, num_wann), dtype=np.cdouble)

        configuration_average_dict[hopping_vector][from_orbital][to_orbital] = strength

    hr_in.close()

    if i == first_point:
        # define dictionary to contain average, calculated as a rolling average
        # rolling average works as long as all wannier90_hr.dat files contain exactly the same hopping vectors
        # this is the case for a set of points generated with Caesar
        average_dict = deepcopy(configuration_average_dict)
    else:
        # calculate rolling average - use point number to determine how many elements went into the current average
        for hop_vector in average_dict.keys():
            average_dict[hop_vector] = ((i-first_point)*average_dict[hop_vector]+configuration_average_dict[hop_vector])/(i-first_point+1)

# open output file
configuration_average_hr = open("configuration_average_hr.dat", "w")

# write out preample and empty average_lines to then write the correction vector by vector
configuration_average_hr.write("\n".join(average_lines))
configuration_average_hr.write("\n")
average_lines  = []

for hop_vector, hop_matrix in average_dict.items():
    for i in range(num_wann):
        for j in range(num_wann):
            average_lines.append("   {}   {}   {}   {}   {}   {}   {}".format(hop_vector[0], hop_vector[1], hop_vector[2], j+1, i+1,
    ...
    # clear average_lines for new elements to be added to it
    average_lines = []

configuration_average_hr.close()
