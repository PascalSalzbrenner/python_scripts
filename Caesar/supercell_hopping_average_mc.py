# script to average the wannier90 hopping parameters calculated for different on average equivalent positions of a supercell generated using
# the in-house Caesar code
# run in directory containing the configurations directory and the supercell.dat file

# input files
# wannier90_hr.dat for each configuration; wannier90.win for static
...
# output
# primitive_cell_hr.dat: hopping elements averaged and mapped to the primitive orbitals
# primitive_cell.win: contains the same information as the static wannier90.win, mapped to the primitive cell
# primitive_cell_atomic_centres.xyz: contains the WCCs in the primitive cell, enforced to the positions of the atoms
# primitive_cell_static_centres.xyz: contains the WCCs in the primitive cell, copied directly from the static wannier90_centres.xyz file

# NB: the output will not be useful for wannier interpolation with wannier90 - this would require new .mmn and .amn files
# it can be imported and manipulated using tight-binding packages such as
# TBmodels: http://z2pack.ethz.ch/tbmodels/doc/1.3/index.html
# PythTB: https://www.physics.rutgers.edu/pythtb/about.html
# TiPSi: http://www.edovanveen.com/tipsi/
# WannierTools, for calculation of topological quantities: http://www.wanniertools.com/index.html
# follow the relevant instructions for your tool of choice
...
# each element of this list will be one line to write to the primitive_cell_hr.dat file
hr_lines = []

# comment line
hr_lines.append("constructed by averaging elements in an _hr.dat file for a {}x{}x{} supercell".format(supercell_size[0],supercell_size[1],
                                                                                                       supercell_size[2]))
...
with open("configurations/{}/wannier90_files/primitive_cell.win".format(point), "w") as win_out:
     win_out.write("\n".join(win_lines))

################################################## write _centres.xyz file ##################################################

# NB: two possibilities are to enforce these as the atomic positions of the static lattice, or the _centres.xyz for the static lattice
# as the static lattice is expected to be the average of the dynamic configurations
# implementing both, as I think they are each useful for different purposes
# enforcing the atomic positions I think might be quite useful for the TB model
# I think the (an) advantage of the latter is that it allows for charge distribution analysis
...
xyz_static_lines.append(str(int(num_wann/num_copies + len(atom_pos_primitive_cell))))
xyz_static_lines.append("Wannier centres for the primitive cell, obtained from a {}x{}x{} supercell".format(supercell_size[0],
                                                                                                      supercell_size[1], supercell_size[2]))
...
# add the first num_atoms*orbitals_per_atom WCCs from the static xyz file to xyz_static_lines
with open("configurations/static/wannier90_files/wannier90_centres.xyz", "r") as xyz_in:
    ...
    for i in range(outer_range):
        ...
        for atom, num_orbitals in orbitals_per_atom.items():
            ...
            for j in range(int(len(atom_pos_primitive_cell[atom])*num_copies*num_orbitals/outer_range)):
                # of these, the first num_primitive*orbitals_per_atom(/2, if spinors == True) lines should be written to the new xyz file
                line = xyz_in.readline()
                if j < len(atom_pos_primitive_cell[atom])*num_orbitals/outer_range:
                    xyz_static_lines.append(line.rstrip("\n"))
...
for atom, positions in atom_pos_primitive_cell.items():
    for position in positions:
        ...
        xyz_static_lines.append("{}    {}  {}  {}".format(atom, position[0], position[1], position[2]))

with open("configurations/{}/wannier90_files/primitive_cell_atomic_centres.xyz".format(point), "w") as xyz_out:
    xyz_out.write("\n".join(xyz_atomic_lines))

with open("configurations/{}/wannier90_files/primitive_cell_static_centres.xyz".format(point), "w") as xyz_out:
    xyz_out.write("\n".join(xyz_static_lines))

################################################## read, process, and write _hr.dat data ##################################################

# read data from wannier90_hr.dat file
hr_in = open("configurations/{}/wannier90_files/wannier90_hr.dat".format(point), "r")
...
with open("configurations/{}/wannier90_files/primitive_cell_hr.dat".format(point), "w") as hr_out:
    ...
    # iterate over all hopping elements and write them to primitive_cell_hr.dat file
