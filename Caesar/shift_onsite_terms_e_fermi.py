# script to rigidly shift all onsite terms of a wannier90_hr.dat tight-binding model by the Fermi energy
...
# output file
# wannier90_fermi_shifted_hr.dat - the corrections at each hopping term, written in the same directory as the original wannier90_hr.dat
...
# second input argument: Fermi energy in eV
e_fermi = float(sys.argv[2])
...
shifted_lines = ["wannier90 TB model with onsite terms shifted by {} eV".format(e_fermi)] + shifted_lines
shifted_hr.write("\n".join(shifted_lines))
shifted_hr.write("\n")
...
for hop_vector, hop_matrix in configuration_hopping_dict.items():
    if hop_vector == ("0", "0", "0"):
        ...
        shifted_lines = to_hr_list({hop_vector: hop_matrix-e_fermi*np.identity(num_wann)})
    else:
