...
internal_energies_plus_PV = [] # [eV/atom], sixth column (column 8 minus the KE)
total_energies = [] # [eV/atom], eighth column (total energy when p=0, enthalpy otherwise)
...
for line in trackfile:
    ...
    if float(data[0]) >= initial_step and (not final_step or float(data[0]) <= final_step):
        ...
        internal_energies_plus_PV.append(float(data[5]))
...
internal_energies_plus_PV = np.array(internal_energies_plus_PV)
total_energies = np.array(total_energies)

average_volume = volumes.mean()
average_temperature = temperatures.mean()
average_pressure = pressures.mean()
average_internal_energy_plus_PV = internal_energies_plus_PV.mean()
...
internal_energy_plus_PV_std = internal_energies_plus_PV.std()
